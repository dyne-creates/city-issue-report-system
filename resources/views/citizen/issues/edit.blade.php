        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg border-t-4 border-violet-500">
                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <div class="mb-6">
                        <h3 class="text-lg font-medium">{{ __('Update Your Report') }}</h3>
                        <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
                            You can only edit reports while they are still in <strong>Reported</strong> status.
                        </p>
                    </div>

                    <form action="{{ route('citizen.issues.update', $issue->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6 max-w-3xl">
                            ...
                            <select id="barangay_id"
                                ...
                                @foreach ($barangays as $barangay)
                                    <option value="{{ $barangay->id }}"
                                        {{ old('barangay_id', $issue->barangay_id) == $barangay->id ? 'selected' : '' }}>
                                        {{ $barangay->name }}
                                    </option>
                                @endforeach
                        ...
                        <div>
                            ...
                            <x-text-input
                                ...
                                type="text"
                                class="mt-1 block w-full"
                                value="{{ old('specific_location', $issue->specific_location) }}"
                                placeholder="Example: Near Session Road"
                            />
                        ...
                        {{-- PHOTO --}}
                        <div>
                            <x-input-label for="photo" :value="__('Photo Evidence (Optional Update)')" />
                            ...
                            @if($issue->photo_path)
                                <div class="mt-3">
                                    ...
                                    <img src="{{ asset('storage/' . $issue->photo_path) }}"
                                         class="w-64 rounded-lg border border-violet-200 shadow"
                                         alt="Current Image">
                                </div>
                            @endif
                        ...
                        <div class="flex items-center justify-end gap-4 pt-4 border-t border-gray-100 dark:border-gray-700">
                            ...
                            <button type="submit" class="inline-flex items-center px-4 py-2 bg-violet-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase hover:bg-violet-700">
                                Update Report
                            </button>
